...
<script>
    $( document ).ready(function(){
        $('#homepage_contents__products .select2').first().select2({
            ajax: {
                url: "{{url('/api/select2/products')}}",
                dataType: 'json'
        ...
        @if($edit)
            ...
            $.ajax({
                url: "{{url('/api/select2/products/initial')}}",
                type: "POST",
                data:{data:"{{json_encode($dataTypeContent->products)}}"},
                success: function(response){
                    for (var key in response.data) {
                        var data =response.data[key];
                        console.log(data);
                        var option = new Option(data.name, data.id, true, true);
                        $('#homepage_contents__products .select2').first().append(option).trigger('change');
                    }
