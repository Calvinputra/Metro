<?php
$parent          = $dataType->model_name::where($primary_parent_field,$dataTypeContent->{$primary_parent_field})->first();
...
$child = $parent?$dataType->model_name::where($child_connection, $parent->{$primary_parent_field} ?? 0)->get():collect();
// edit view
            ...
            if ($edit) {
                ...
                <?php foreach ($child as $key => $cld): ?>
                    ...
                    // Use withTrashed() if model uses SoftDeletes and if toggle is selected
                        ...
                        $dataTypeContent = call_user_func([$model, 'findOrFail'], $cld->{$child_primary} ?? 0);

                    } else {
                    // If Model doest exist, get data from table name
                        $dataTypeContent = DB::table($dataType->name)->where('id', $cld->{$child_primary} ?? 0)->first();
                    }
                    ?>
                    @include('custom_field_component.edit-add_transaction_detail_row',[
                    "counter"=>$cld->{$child_primary},
                    "child"=>$cld,
                    ])
                <?php endforeach?>
